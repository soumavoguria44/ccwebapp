...
aws cloudformation create-stack --stack-name $PolicyStack --template-body file://csye6255-cf-cicd.json --capabilities CAPABILITY_NAMED_IAM --parameters ParameterKey=CircleCIUser,ParameterValue=$UserId ParameterKey=S3CodeBucket,ParameterValue=$bucketName
